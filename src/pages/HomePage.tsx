  ...
  return (
    ...
    >
      <div className="relative h-full overflow-hidden rounded-[2rem] border border-white/12 bg-[#070910] p-3 shadow-[0_34px_120px_rgba(0,0,0,0.55)]">
        <div className={`relative flex h-full flex-col overflow-hidden rounded-[1.45rem] border border-white/10 bg-gradient-to-br ${project.color} p-5`}>
          <div className="absolute -left-20 top-1/3 h-52 w-52 rounded-full bg-cyan-100/10 blur-3xl" />
          <div className="absolute -right-24 -top-24 h-64 w-64 rounded-full bg-white/12 blur-3xl" />
          <div className="absolute inset-x-4 top-1/2 h-px bg-gradient-to-r from-transparent via-white/28 to-transparent" />
          ...
          <div className="relative flex-1 flex flex-col justify-between">
            <div>
              <CodeMark className="mb-4 text-3xl text-white/58" />
              <h3 className="text-3xl font-semibold tracking-[-0.05em] line-clamp-2">{project.title}</h3>
            </div>
            <div>
              <p className="text-sm leading-6 text-white/68 mb-4 line-clamp-3">{project.summary}</p>
              <div className="flex flex-wrap gap-2 mb-4">
                {project.stack.slice(0, 3).map((tag) => (
                  <span key={tag} className="rounded-full bg-black/30 px-2 py-1 text-xs text-white/72">{tag}</span>
                ))}
              </div>
              <button
                onClick={() => onOpen(project)}
                className="w-full inline-flex items-center justify-center gap-2 rounded-full bg-white px-4 py-2 text-xs font-semibold text-black transition hover:scale-[1.02]"
              >
                Open Case Study <FiArrowUpRight className="w-3 h-3" />
              </button>
            </div>
          </div>
        </div>
...
const MobileProjectIndicators = ({ activeIndex, total }: { activeIndex: number; total: number }) => {
  return (
    <div className="flex justify-center items-center gap-2 mt-4">
      {Array.from({ length: total }).map((_, i) => (
        <motion.div
          key={i}
          animate={{
            scale: i === activeIndex ? 1.2 : 0.8,
            backgroundColor: i === activeIndex ? '#67e8f9' : 'rgba(255,255,255,0.2)',
          }}
...
const ProjectStack = ({ onOpen }: { onOpen: (project: Project) => void }) => {
  ...
  useEffect(() => {
    // Only subscribe to scroll on desktop
    if (isMobile) return
    ...
    return unsubscribe
  }, [activeValue, manualIndex, isMobile])
  ...
  return (
    <section ref={sectionRef} id="projects" className="relative z-10 px-5 py-6 md:px-8" style={{ minHeight: isMobile ? 'auto' : '175vh' }}>
      <div className={`${isMobile ? '' : 'sticky top-4 flex min-h-[calc(100vh-2rem)] items-center py-6'}`}>
        <div className="mx-auto w-full max-w-6xl">
          <SectionTitle eyebrow="Projects" title={isMobile ? "Swipe through selected builds." : "Scroll through a stacked deck of selected builds."} />
          ...
          {isMobile && (
            <div>
              ...
              <MobileProjectIndicators activeIndex={activeIndex} total={projects.length} />
              <div className="flex justify-center gap-4 mt-6">
                <button
                  onClick={handleSwipeRight}
                  disabled={activeIndex === 0}
                  className="rounded-full border border-white/12 bg-white/[0.05] px-4 py-2 text-xs font-semibold text-white/64 hover:text-white disabled:opacity-40 transition"
                >
                  ← Previous
                </button>
                <button
          ...
          <div className="space-y-12">
            {groupedSkills.map(({ group, items }) => (
              <motion.div key={group} variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }}>
                ...
                <div className="flex overflow-x-auto md:grid md:grid-cols-2 gap-4 md:overflow-x-visible lg:grid-cols-4 pb-2 md:pb-0 -mx-5 md:mx-0 px-5 md:px-0 snap-x snap-mandatory">
                  {items.map((skill) => (
                    <motion.article
                      key={skill.name}
                      variants={reveal}
                      whileHover={{ y: -8, rotateX: 6, scale: 1.025 }}
                      className="group rounded-3xl border border-white/10 bg-white/[0.045] p-5 backdrop-blur-xl transition hover:border-white/24 hover:shadow-[0_0_44px_rgba(103,232,249,0.14)] flex-shrink-0 w-[calc(100vw-50px)] md:w-auto md:flex-shrink snap-center"
                    >
